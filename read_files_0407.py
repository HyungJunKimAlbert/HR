...
for p in pid_list:
    ...
    for idx, i in enumerate(base_path.glob(f'{p}*.png')):
        ...
        mhr_box_color = color[fhr_yt_new:fhr_yb_new, xl_new:xr_new]     # MHR 코드 추가 (3/14)
        # mhr_box_color = color[250:470, xl_new:xr_new]     # MHR 코드 추가 (3/24) >>> Twin 환자는 이 좌표로... 
    ...
    # uc = list(tmp_uc['uc'])
    ''' twin's MHR : 75 미만만 추출할 것 : Twin red line 과 blue line 이 겹치면서 MHR과 동일한 픽셀이 생성되는 경우때문에 조건 걸어줘야함.'''
